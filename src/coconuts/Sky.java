...
// the sky manages all island objects (they are all "in" the sky somewhere...)
//   including detecting when objects hit
public class Sky {
    ...
    public Sky(int height, int width, Pane skyPane) {
        this.height = height;
        this.width = width;
        this.skyPane = skyPane;

        this.theCrab = new Crab(this, height, width);
        registerObject(theCrab);
        skyPane.getChildren().add(theCrab.getImageView());
    ...
    public void tryDropCoconut() {
        if (gameTick % DROP_INTERVAL == 0 && theCrab != null) {
            ...
            registerObject(c);
            skyPane.getChildren().add(c.getImageView());
        }
        gameTick++;
    }
        ...
        for (IslandObject thisObj : allObjects) {
            for (HittableIslandObject hittableObject : hittableIslandSubjects) {
                if (thisObj.canHit(hittableObject) && thisObj.isTouching(hittableObject)) {
                    ...
                    skyPane.getChildren().remove(hittableObject.getImageView());
                }
            }
        }
